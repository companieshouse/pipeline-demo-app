#!/bin/bash
#
# Start script for pipeline-demo-app
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/pipeline-demo-app/env

    PORT=${PIPELINE_DEMO_APP_PORT:=8080}
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/pipeline-demo-app.jar"
